...
class SimpleDetector(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.cls_head(cls_features), torch.sigmoid(self.bbox_head(bbox_features))


def calc_giou(pred, target):
...
def giou_loss(pred, target):
    return 1 - calc_giou(pred, target)


def detection_loss(cls_pred, bbox_pred, cls_targets, bbox_targets, lambda_bbox=80.0):
...
# root = BASE_DIR / "shapes_dataset"
# root = BASE_DIR / "shapes_dataset_bg"
root = BASE_DIR / "shapes_dataset_random"
...
if save_path.exists():
    ...
    for epoch in range(1, epochs + 1):
        ...
        with torch.no_grad():
            for images, cls_t, bbox_t in val_loader:
                ...
                val_loss += loss.item()
                val_giou += calc_giou(bbox_pred, bbox_t).item()
                correct += (cls_pred.argmax(1) == cls_t).sum().item()
                total += cls_t.size(0)

        val_acc = correct / total
        val_giou /= len(val_loader)
        ...
        if val_acc > best_acc or (val_acc == best_acc and val_giou > best_giou):
            best_acc = val_acc
            best_giou = val_giou
            no_improve = 0
        ...
        if (val_acc >= accuracy_threshold and val_giou >= giou_threshold) or no_improve >= no_improve_count:
            break
    ...
    plt.plot(history["val_giou"], label="val giou", color="red")
    plt.legend()
